...
const createMyCustomer = async(req: Request, res: Response) => {
    ...
    try{
        ...
        //const verificationLink = `http://167.88.43.130/nadarfront/auth/verify-email?token=${token}&byAssistant=true`;
        const verificationLink = `https://clientes.nadaresvidaperu.pe/nadarfront/auth/verify-email?token=${token}&byAssistant=true`;
        const bodyHTML = `
            <div>
